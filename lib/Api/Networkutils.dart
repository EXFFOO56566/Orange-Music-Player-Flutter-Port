...
class Networkutils {
  ...
  static final BASEURL = "http://tochycomputerservices.com/OrangeMusicPlayer/API/";

  // ignore: non_constant_identifier_names
  static final Baserl1 = "http://tochycomputerservices.com/OrangeMusicPlayer/";
  ...
  Future getBannerSlider() async {
    await post(BASEURL + 'home', body: {
      ...
      'user_id': '10',
      // for a different screen? nice config
      //'is_myProfile': '3',
    }).then((value) {
  ...
  downloads() async {
    await post(BASEURL + 'isAllowDownloads', body: {"user_id": '10'})
        .then((dynamic res) async {
  ...
  Future deleteplaylist(String id) async {
    await post(BASEURL + 'deletePlayList',
        body: {'user_id': '10', 'user_playlist_id': id});
  }
  ...
  Future like(String id, String liketype, String liketypeId) async {
    await post(BASEURL + 'like', body: {
      //specify user id like '3' to aviod errors
      'user_id': id,
      'like_type': liketype,
